...
function windowsDriver(shellPath, overrides) {
  ...
  function makeServer() {
    const srv = net.createServer((sock) => {
      channel = sock;
      sock.on('error', () => {});
      // Only clear the channel if this socket is still the current one. After a
      // kill the replacement helper can connect before the dead socket's close
      // fires, and clearing unconditionally would strand every later command in
      // the queue forever.
      sock.on('close', () => { if (channel === sock) channel = null; });
  ...
  return {
    ...
    start(cwd, handlers, token) {
      ...
      // Drop the channel the moment the helper dies, BEFORE the core is told.
      // Its socket stays nominally open until the close event lands a tick
      // later, and a command written in that window vanishes into a dead pipe -
      // which is exactly what the next run() after a kill would do.
      const forgetChannel = () => {
        if (!channel) return;
        try { channel.destroy(); } catch (e) {}
        channel = null;
      };
      child.on('exit', forgetChannel);
      child.on('error', forgetChannel);
